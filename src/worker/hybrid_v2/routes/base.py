...
class BaseRouteHandler:
    ...
    async def _search_via_entity_graph(
        ...
        def _run_sync():
            ...
            // Support both Sentence-based and TextChunk-based MENTIONS
            ...
            WITH e, CASE WHEN src:TextChunk THEN src ELSE NULL END AS direct_chunk, src
            OPTIONAL MATCH (src)-[:PART_OF]->(parent_chunk:TextChunk {{group_id: $group_id}})
            WHERE direct_chunk IS NULL
            WITH e, coalesce(direct_chunk, parent_chunk) AS t
            WHERE t IS NOT NULL
