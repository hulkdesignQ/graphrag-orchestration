"""Route 9: HippoRAG 2 + Community-Seeded PPR (experimental).
...
Key differences from Route 7 default preset:
  ...
  - ROUTE_NAME = "route_9_hipporag2_experimental"
  - Env-var prefix: ROUTE9_* (falls back to ROUTE7_* then defaults)
...
class HippoRAG2ExperimentalHandler(BaseRouteHandler):
    """Route 9: HippoRAG 2 + Community-Seeded PPR (experimental).
    ...
    async def execute(
        ...
        # Priority: config_overrides → ROUTE9_env → preset value → ROUTE7_env → default.
        ...
        # comprehensive_search preset).  Use ROUTE9_ env vars to override preset.
        def _ov(key: str, env_var: str, default: str) -> str:
            val = _co.get(key)
            if val:
                return val
            if env_var.startswith("ROUTE7_"):
                r9_val = os.getenv("ROUTE9_" + env_var[7:])
                if r9_val is not None:
                    return r9_val
            elif env_var.startswith("ROUTE9_"):
                r9_val = os.getenv(env_var)
                if r9_val is not None:
                    return r9_val
            if key in preset:
                return default
        ...
        rerank_instruction = _ov(
            "rerank_instruction", "ROUTE9_RERANK_INSTRUCTION",
        ...
        community_passage_seeds_enabled = _ov(
            "community_passage_seeds", "ROUTE9_COMMUNITY_PASSAGE_SEEDS",
        ...
        community_guided_enabled = _ov(
            "community_guided_instruction", "ROUTE9_COMMUNITY_GUIDED_INSTRUCTION",
        ...
        neural_gate_threshold = float(_ov(
            "neural_gate_threshold", "ROUTE9_NEURAL_GATE_THRESHOLD",
        ...
        ppr_community_balance = _ov(
            "community_balanced_seeds", "ROUTE9_COMMUNITY_BALANCED_SEEDS",
            "1" if preset.get("community_balanced_seeds", False) else "0"
        ).strip().lower() in {"1", "true", "yes"}
        ppr_community_balance_alpha = float(_ov(
            "community_balance_alpha", "ROUTE9_COMMUNITY_BALANCE_ALPHA",
        ...
        ppr_community_walk_penalty = _ov(
            "community_walk_penalty", "ROUTE9_COMMUNITY_WALK_PENALTY",
            "1" if preset.get("community_walk_penalty", False) else "0"
        ).strip().lower() in {"1", "true", "yes"}
        ppr_community_walk_alpha = float(_ov(
            "community_walk_alpha", "ROUTE9_COMMUNITY_WALK_ALPHA",
        ...
        propagation_mode = _ov(
            "propagation_mode", "ROUTE9_PROPAGATION_MODE",
            preset.get("propagation_mode", "ppr")
        ).strip().lower()
        gpr_hops = int(_ov("gpr_hops", "ROUTE9_GPR_HOPS",
                           str(preset.get("gpr_hops", 10))))
        gpr_profile = _ov(
            "gpr_profile", "ROUTE9_GPR_PROFILE",
            preset.get("gpr_profile", "ppr")
        ).strip().lower()
        gpr_weights_csv = _ov(
            "gpr_weights", "ROUTE9_GPR_WEIGHTS",
        ...
        appnp_hub_penalty_mode = _ov(
            "appnp_hub_penalty_mode", "ROUTE9_APPNP_HUB_PENALTY_MODE",
            str(preset.get("appnp_hub_penalty_mode", "none"))
        ).strip().lower()
        appnp_hub_penalty_alpha = float(_ov(
            "appnp_hub_penalty_alpha", "ROUTE9_APPNP_HUB_PENALTY_ALPHA",
            str(preset.get("appnp_hub_penalty_alpha", 1.0))
        ))
        appnp_hub_penalty_base = float(_ov(
            "appnp_hub_penalty_base", "ROUTE9_APPNP_HUB_PENALTY_BASE",
        ...
        appnp_norm_mode = _ov(
            "appnp_norm_mode", "ROUTE9_APPNP_NORM_MODE",
        ...
        map_reduce_synthesis = _ov(
            "map_reduce_synthesis", "ROUTE9_MAP_REDUCE_SYNTHESIS",
            "1" if preset.get("map_reduce_synthesis", False) else "0"
        ).strip().lower() in {"1", "true", "yes"}
        map_reduce_concurrency = int(_ov(
            "map_reduce_concurrency", "ROUTE9_MAP_REDUCE_CONCURRENCY", "8"
        ))
        ...
        llm_dedup_enabled = _ov(
            "llm_dedup", "ROUTE9_LLM_DEDUP",
            "1" if preset.get("llm_dedup", False) else "0"
        ).strip().lower() in {"1", "true", "yes"}
        llm_dedup_model = _ov(
            "llm_dedup_model", "ROUTE9_LLM_DEDUP_MODEL",
        ...
        min_chunks_per_doc = int(_ov(
            "min_chunks_per_doc", "ROUTE9_MIN_CHUNKS_PER_DOC",
        ...
        entity_emb_seeds_enabled = _ov(
            "entity_embedding_seeds", "ROUTE9_ENTITY_EMBEDDING_SEEDS", "1"
        ).strip().lower() in {"1", "true", "yes"}
        ...
        if entity_emb_seeds_enabled and self._async_neo4j:
            try:
                ...
                entity_emb_top_k = int(_ov(
                    "entity_embedding_top_k", "ROUTE9_ENTITY_EMBEDDING_TOP_K", "10"
                ))
                entity_emb_weight = float(_ov(
                    "entity_embedding_weight", "ROUTE9_ENTITY_EMBEDDING_WEIGHT", "0.3"
                ))
        ...
        entity_seed_expansion_enabled = _ov(
            "entity_seed_expansion", "ROUTE9_ENTITY_SEED_EXPANSION", "0"
        ).strip().lower() in {"1", "true", "yes"}
        expansion_min_sim = float(_ov(
            "entity_seed_expansion_min_sim", "ROUTE9_ENTITY_SEED_EXPANSION_MIN_SIM", "0.60"
        ))
        expansion_max_per_seed = int(_ov(
            "entity_seed_expansion_max_per_seed", "ROUTE9_ENTITY_SEED_EXPANSION_MAX_PER_SEED", "3"
        ))
        expansion_base_weight = float(_ov(
            "entity_seed_expansion_weight", "ROUTE9_ENTITY_SEED_EXPANSION_WEIGHT", "0.10"
        ))
        ...
        entity_doc_map_enabled = _ov(
            "entity_doc_map", "ROUTE9_ENTITY_DOC_MAP", "0"
        ).strip().lower() in {"1", "true", "yes"}
        ...
        if rerank_enabled and passage_scores:
            ...
            # Route 9 addition: inject community-sourced sentence IDs directly
        ...
        diversity_min = int(_ov("community_diversity_min", "ROUTE9_COMMUNITY_DIVERSITY_MIN", "3"))
        ...
        entity_bridge_enabled = _ov(
            "entity_bridge_filter", "ROUTE9_ENTITY_BRIDGE_FILTER",
        ...
        if entity_bridge_enabled and self._ppr_engine and passage_scores and triple_entity_ids:
            ...
            reranker_keep_n = int(_ov(
                "entity_bridge_reranker_keep", "ROUTE9_ENTITY_BRIDGE_RERANKER_KEEP",
                "5"
            ))
            ...
            bridge_coverage_min = float(_ov(
                "entity_bridge_coverage_min", "ROUTE9_ENTITY_BRIDGE_COVERAGE_MIN",
                "0.30"
            ))
        ...
        max_per_doc = int(_ov(
            "max_chunks_per_doc", "ROUTE9_MAX_CHUNKS_PER_DOC",
        ...
        dedup_threshold = float(_ov(
            "semantic_dedup_threshold", "ROUTE9_SEMANTIC_DEDUP_THRESHOLD",
        ...
        # Route 9: Inject community sentence texts directly into synthesis context.
        ...
        community_inject_enabled = _ov(
            "community_context_inject", "ROUTE9_COMMUNITY_CONTEXT_INJECT", "1"
        ).strip().lower() in {"1", "true", "yes"}
        if community_inject_enabled and community_passage_seeds_enabled and community_sentence_ids and pre_fetched_chunks is not None:
            ...
            if community_extra_ids:
                try:
                    inject_cap = int(_ov("community_inject_cap", "ROUTE9_COMMUNITY_INJECT_CAP", "100"))
        ...
        entity_inject_enabled = _ov(
            "entity_context_inject", "ROUTE9_ENTITY_CONTEXT_INJECT",
