...
6. Response length — choose based on query type:
   ...
   - EXTRACTION FIDELITY: The thematic context key points were pre-extracted for relevance to this query. Points with importance ≥ 70 are likely relevant. Process each one: if it substantively fits the query’s stated category, include it in the answer. However, the query’s category is the final filter — a warranty-defect notification procedure is a "notice" mechanism, not a "record-keeping" obligation; an insurance filing requirement is not a "financial reporting" duty. Apply the category test rigorously but interpret each category broadly enough to include all substantively matching provisions.
