...
"""Route 4 benchmark questions executed against Route 6 (Concept Search).

This benchmark runs the same Q-D (drift/multi-hop) positive questions and
Q-N (negative) questions from the Route 4 benchmark against Route 6's
concept_search route, enabling a direct head-to-head comparison.

Route 6 uses community-based concept search without PPR.

Endpoint:
  POST /hybrid/query  (force_route=concept_search)
...
Usage
  python3 scripts/benchmark_route6_concept_r4_questions.py \\
...
FORCE_ROUTE = "concept_search"   # Route 6
ROUTE_LABEL = "route6_concept"
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Skip authentication (for local testing)",
    )
    parser.add_argument(
        "--positive-prefix",
        type=str,
        default="Q-D",
        help="Positive question ID prefix (default: Q-D). Use Q-G for community/thematic questions.",
    )
    parser.add_argument(
        "--negative-prefix",
        type=str,
        default="Q-N",
        help="Negative question ID prefix (default: Q-N)",
    )
    ...
    # Read questions
    pos_prefix = args.positive_prefix
    neg_prefix = args.negative_prefix
    questions = read_question_bank(qbank_path, positive_prefix=pos_prefix, negative_prefix=neg_prefix)
    ...
    positive_count = sum(1 for q in questions if q.qid.startswith(pos_prefix))
    negative_count = sum(1 for q in questions if q.qid.startswith(neg_prefix))
    print(f"  Positive tests ({pos_prefix}): {positive_count}")
    print(f"  Negative tests ({neg_prefix}): {negative_count}")
    ...
    qset_label = f"{pos_prefix.lower()}questions"
    scenario_name = f"{ROUTE_LABEL}_{qset_label}{wp_tag}_{args.response_type}"
    ...
    out_json = out_dir / f"{ROUTE_LABEL}_{qset_label}{suffix}_{timestamp}.json"
    out_md = out_dir / f"{ROUTE_LABEL}_{qset_label}{suffix}_{timestamp}.md"
